export const NEC_API_KEY = import.meta.env.VITE_NEC_API_KEY ?? '';
...
export const SIDO_NAME_ALIASES: Record<string, string[]> = {
  '32': ['강원특별자치도', '강원도'],
  '35': ['전북특별자치도', '전라북도'],
};

export const SIGUNGU_NAME: Record<string, string> = {
...
export const PARENT_CITY: Record<string, string> = {
...
export function getApiSdName(sidoCd: string, sgId: string): string {
  const year = parseInt(sgId.slice(0, 4), 10);
  if (SIDO_NAME_ALIASES[sidoCd]) {
...
export function getSidoNameByCode(admCd: string, sgId: string): string {
  return getApiSdName(admCd.slice(0, 2), sgId);
}

export function getSigunguNameByCode(admCd: string): string {
  return SIGUNGU_NAME[admCd.slice(0, 5)] ?? '';
}

export function getNecQueryRegion(admCd: string, sgId: string) {
  const sidoCd = admCd.slice(0, 2);
  const sigunguCd = admCd.slice(0, 5);
  const sdName = getApiSdName(sidoCd, sgId);
  const rawSigungu = SIGUNGU_NAME[sigunguCd] ?? '';
  const parentCity = PARENT_CITY[sigunguCd];
  const sggName = parentCity ?? rawSigungu;
  ...
  return {
    ...
    sdName,
    sggName,
    displaySggName: rawSigungu || sggName,
    parentCity,
  };
}
